  ...
  useEffect(() => {
    const fetchLatestZoneId = async () => {
      try {
        ...
        const response = await fetch("http://localhost:8000/api/zones/last-zone-id");
        
        if (response.ok) {
          const data = await response.json();
          // Set next zone ID to be latest ID + 1, or 1 if no zones exist (null case = 0)
          setNextZoneId((data.last_zone_id || 0) + 1);
        } else {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/cameras", {
        ...
        body: JSON.stringify({
          ...cameraData,
          zones: zones.map(z => ({
            id: parseInt(z.id), // Convert string ID back to number for API
            type: z.type,
            coordinates: z.coordinates,
            name: z.name
          })),
          lane_directions: laneDirections.map(dir => ({
            from_zone_id: parseInt(dir.fromZoneId),
            to_zone_id: parseInt(dir.toZoneId),
            name: dir.name
          })),
          light_zone_mappings: lightZoneMappings.map(mapping => ({
            light_zone_id: parseInt(mapping.lightZoneId),
            lane_zone_id: parseInt(mapping.laneZoneId)
          }))
        })
      });
